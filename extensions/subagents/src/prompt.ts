...
export const SUBAGENT_SPAWN_TOOL_DESCRIPTION =
  "Spawn a background subagent with its own context window. Choose pi (default in-process Pi session), Claude Code, or Codex CLI. Completed Pi children are disposable by default; set persistent=true only when the child needs a later turn or takeover. Pi children use a durable role profile; omit role to use worker. Fire-and-forget: this returns immediately with an id. Results arrive automatically or through subagent_wait. Children cannot orchestrate more agents/workflows or ask the user, and cannot see this conversation, so the prompt must be self-contained. Max 8 subagents can run at once.";
...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  persistent:
    "Keep a completed Pi child session available for later subagent_send turns or takeover. Defaults to false for Pi. Claude and Codex keep their existing persistent lifecycle.",
};
...
/** Describes aborting running subagents and their lifecycle after settlement. */
export const SUBAGENT_CANCEL_TOOL_DESCRIPTION =
  "Interrupt one or more running subagents. Persistent sessions remain available for subagent_send; disposable Pi sessions release when interruption settles.";
...
export const SUBAGENT_SEND_TOOL_DESCRIPTION =
  "Send a message to a subagent. This steers any active turn, or starts a new turn when its persistent session is idle.";
